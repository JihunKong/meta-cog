...
위 데이터를 분석하여 ${user.name} 학생에게 맞춤형 학습 조언을 제공해주세요.

## 응답 가이드라인
1. 간결하고 실용적인 조언을 제공해주세요 (너무 길지 않게).
2. 실제 학습 데이터에 기반하여 개인화된 내용을 만들어주세요.
3. 형식에 얽매이지 말고 학생에게 가장 도움이 될 내용을 자유롭게 작성해주세요.
4. 각 조언에는 실행 가능한 구체적인 단계를 포함해주세요.
5. 다음 카테고리로 구분하되, 형식은 자유롭게 작성해도 됩니다:

## STRATEGY
학생의 학습 패턴, 과목별 성취도, 진도율 등을 고려하여 맞춤형 학습 전략을 추천해주세요. 

## SCHEDULE
효과적인 시간 관리와 학습 계획에 관한 조언을 제공해주세요. 구체적인 일정 제안도 좋습니다.

## SUBJECT
특정 과목에 대한 학습 방법을 추천해주세요. 학생이 어려워하는 과목이나 더 발전시킬 수 있는 과목에 초점을 맞추세요.

## UNIT
특정 단원이나 주제에 대한 집중 학습 방법을 제안해주세요.
`;
    ...
    try {
      ...
      const apiRequestPromise = anthropic.messages.create({
        ...
        max_tokens: 2000,
        system: "당신은 청해FLAME 자기주도학습 관리 시스템의 AI 학습 조언자입니다. 형식에 얽매이지 말고 창의적이고 개인화된 조언을 제공해 주세요. 학생의 학습 데이터를 철저히 분석하고 그에 맞는 실질적인 조언을 해주세요.",
        messages: [
