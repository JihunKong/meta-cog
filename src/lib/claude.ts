...
const anthropic = new Anthropic({
  apiKey: process.env.ANTHROPIC_API_KEY || '', // 환경 변수에서 API 키 가져오기
});
...
`;
    ...
    const message = await anthropic.messages.create({
      model: 'claude-3-7-sonnet-20250219', // Claude 3.7 Sonnet 모델을 사용
      max_tokens: 2000,
    ...
    // API 응답에서 JSON 추출
    // content는 여러 블록으로 구성될 수 있으며 각 블록은 다른 유형일 수 있음
    // 텍스트 타입의 블록만 처리
    let responseText = '';
    ...
    return [
      {
        userId,
        subject: '전체',
        content: '학습 데이터를 분석하는 중 오류가 발생했습니다. 균형 잡힌 학습 계획을 수립하고 규칙적으로 학습하세요.',
        type: 'STRATEGY',
      },
    ];
  }
