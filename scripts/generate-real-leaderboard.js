...
async function generateRealLeaderboard() {
  console.log('🚀 실제 데이터 기반 리더보드 생성 시작...', new Date().toISOString());
  ...
  try {
    ...
    for (const period of periods) {
      ...
      for (const userDoc of studentsSnapshot.docs) {
        ...
        try {
          ...
          console.log(`  점수: ${scoreData.score}`);
      ...
      leaderboardData.slice(0, 5).forEach((entry, index) => {
        console.log(`  ${entry.rank}위: ${entry.name} (${entry.score}점)`);
      });
    ...
    console.log(`\n🎉 총 ${totalGenerated}개의 실제 데이터 리더보드 생성 완료!`);

  } catch (error) {
    console.error('❌ 실제 리더보드 생성 오류:', error);
  }
}
